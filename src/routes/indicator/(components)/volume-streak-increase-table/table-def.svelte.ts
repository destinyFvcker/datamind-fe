...
import { stockRankCxflThs } from '$lib/api/indicator';
...
import type { VolumeStreakIncreaseData } from '../../(data)/data';

export const tableStatus: {
	data: VolumeStreakIncreaseData[];
...
export async function updateData() {
	...
	const res = await stockRankCxflThs();
	if (res) {
...
const columnHelper = createColumnHelper<VolumeStreakIncreaseData>();
...
export const defaultColumns = [
	...
	columnHelper.accessor('volume_increase_days', {
		header: () => {
			return renderSnippet(cellCenter, '放量天数');
		},
		cell: (props) => {
...
export const options: TableOptions<VolumeStreakIncreaseData> = {
	get data() {
